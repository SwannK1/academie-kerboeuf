...
// ─── Système de couleurs par personnage ──────────────────────────────────────
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!professor) return { title: "Personnage introuvable | Académie Kerboeuf" };
  return {
...
export default async function PersonnagePage({ params }: PageProps) {
  ...
  return (
    <main>
      ...
      {related.length > 0 ? <RelatedPersonnagesSection professors={related} /> : null}
...
function RelatedPersonnagesSection({ professors }: { professors: ProfessorProfile[] }) {
  return (
    <section className="px-4 pb-14 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <SectionTitle eyebrow="Dans l'Académie" title="Personnages associés" />
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {professors.map((p) => {
            const pa = ac(p);
            return (
