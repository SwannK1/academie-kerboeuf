...
function PersonnageCard({ professor }: { professor: ProfessorCardData }) {
  const a = ac(professor);
...
export function PersonnageGallery({ professors }: { professors: ProfessorCardData[] }) {
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((professor) => (
            <PersonnageCard key={professor.slug} professor={professor} />
          ))}
        </div>
      </div>
    </section>
