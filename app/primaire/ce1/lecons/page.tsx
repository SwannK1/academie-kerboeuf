...
export const metadata: Metadata = {
  title: "Leçons CE1 | Académie Kerboeuf",
  description:
    "Catalogue des leçons disponibles en CE1 — Cycle 2. Objectifs, compétences et exercices progressifs.",
};

// ── Page ──────────────────────────────────────────────────────────────────────

export default function Ce1LessonsIndexPage() {
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          ...
          <p className="mt-5 max-w-2xl text-lg leading-8 text-muted">
            Prévisualisation des leçons disponibles en CE1. Chaque leçon propose
          ...
          {entries.length === 0 ? (
            <div className="rounded-md border border-white/10 bg-white/[0.025] p-8">
              ...
              <p className="mt-3 text-sm leading-7 text-muted">
                Les leçons CE1 sont en cours de préparation. Revenez bientôt.
              </p>
            </div>
          ) : (
            <ul className="space-y-6" role="list">
              {entries.map(({ lesson, domain, subdomain }) => (
                <li key={lesson.id}>
                  <Link
                    href={`/primaire/ce1/lecons/${lesson.slug}`}
