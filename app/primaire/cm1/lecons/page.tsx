...
export const metadata: Metadata = {
  title: "Leçons CM1 | Académie Kerboeuf",
  description:
    "Catalogue des leçons disponibles en CM1 — Cycle 3. Objectifs, compétences et exercices progressifs.",
};

// ── Page ──────────────────────────────────────────────────────────────────────

export default function Cm1LessonsIndexPage() {
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          ...
          <p className="mt-5 max-w-2xl text-lg leading-8 text-muted">
            Prévisualisation des leçons disponibles en CM1. Chaque leçon propose
          ...
          {entries.length === 0 ? (
            <div className="rounded-md border border-white/10 bg-white/[0.025] p-8">
              ...
              <p className="mt-3 text-sm leading-7 text-muted">
                Les leçons CM1 sont en cours de préparation. Revenez bientôt.
              </p>
            </div>
          ) : (
            <ul className="space-y-6" role="list">
              {entries.map(({ lesson, domain, subdomain }) => (
                <li key={lesson.id}>
                  <Link
                    href={`/primaire/cm1/lecons/${lesson.slug}`}
                    className="group block rounded-md border border-white/10 bg-white/[0.025] p-6 transition hover:border-sky/30 hover:bg-white/[0.04]"
                  >
                    {/* Domaine / Sous-domaine */}
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="inline-flex rounded border border-sky/25 bg-sky/[0.05] px-2.5 py-1 text-xs font-bold text-sky">
                        {domain.title}
                      </span>
                    ...
                    {/* Titre */}
                    <h2 className="mt-4 text-xl font-black text-foreground transition-colors group-hover:text-sky">
                      {lesson.title}
                    </h2>
                    ...
                    <div className="mt-5 flex flex-wrap items-center gap-4 border-t border-white/10 pt-4">
                      ...
                      />
                      <span className="ml-auto text-xs font-semibold text-sky opacity-0 transition-opacity group-hover:opacity-100">
                        Voir la leçon →
                      </span>
                    </div>
                  </Link>
