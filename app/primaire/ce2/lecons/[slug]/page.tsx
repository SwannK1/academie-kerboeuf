...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${ctx.lesson.title} – CE2 | Académie Kerboeuf`,
...
export default async function Ce2LessonPage({ params }: PageProps) {
  const { slug } = await params;
  const ctx = findLessonContext(slug);
  if (!ctx) notFound();
    ...
    <main>
        ...
        <div className="mx-auto max-w-4xl">
          <Breadcrumb
            items={[
              ...
              { label: "Leçons",   href: "/primaire/ce2/lecons"  },
              { label: lesson.title },
            ]}
          />
        </div>
          ...
          <ul className="mt-6 space-y-3" role="list">
            {lesson.successCriteria.map((criterion) => (
              ...
              >
                <span
                  aria-hidden="true"
                  className="mt-0.5 h-5 w-5 shrink-0 rounded border border-jade/40 bg-jade/[0.06]"
                />
            ...
            {lesson.exercises.map((exercise, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Consigne — preserve line breaks for multi-line instructions */}
                  <p className="mt-5 whitespace-pre-line text-base font-semibold leading-7 text-foreground">
                  ...
                  <details className="mt-4 print:hidden">
                    ...
                    <div className="mt-3 rounded-md border border-jade/20 bg-jade/[0.04] p-4">
      ...
      {lesson.characterLink && lesson.characterLink.roleHint && (
        <section className="border-t border-white/10 px-4 py-10 sm:px-6 lg:px-8 print:hidden">
          <div className="mx-auto max-w-4xl">
            <div className="rounded-md border border-jade/20 bg-jade/[0.04] p-5">
              <p className="text-xs font-bold uppercase tracking-[0.22em] text-jade">
