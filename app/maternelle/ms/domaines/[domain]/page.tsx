...
export function generateStaticParams() {
  return msDomains.map((domain) => ({ domain: domain.slug }));
}
...
export async function generateMetadata({
  ...
  const domain = getMsDomainBySlug(domainSlug);
        ...
        <div className="mx-auto max-w-7xl">
          <Breadcrumb
            items={[
              ...
              { label: "Moyenne Section", href: "/maternelle/ms" },
      ...
      <section className="px-4 pb-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          {domain.subdomains?.some((subdomain) =>
                      ...
                      <div className="mt-4 grid gap-3">
                        {sequence.resources?.map((resource) => {
                          ...
                          return (
                            <div
                              key={resource.kind}
                              className="flex flex-wrap items-center justify-between gap-3 rounded border border-ink/10 bg-ink/30 p-3"
                            >
            ...
            <p className="rounded border border-ink/10 bg-ink/[0.025] p-5 text-sm text-muted">
