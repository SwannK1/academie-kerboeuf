...
| --- | ---: |
| PDF audités intégralement (toutes pages, contenu contrôlé) | 365 (33 CP + **28 CE1 — niveau clos** + 24 CE2 + **126 CM1 — niveau clos** + **154 CM2 — niveau intégralement audité, 57/57 compétences**) |
| PDF restant à auditer | 606 |
...
23. **Tous les PDF pédagogiques existants du dépôt sont maintenant audités** (CP 33/33, CE1 28/28, CE2 24/24, CM1 126/126, CM2 154/154 = 365/365 PDF pédagogiques primaire). Il ne reste plus de PDF non audité dans le périmètre CP → CM2. **Prochaine étape à décider avec l'utilisateur** : soit une décision produit (créer du contenu QLM/sciences/EMC là où il manque, statut EMC général), soit étendre le périmètre à un autre niveau/matière hors primaire (collège/lycée, mentionnés dans la mission d'origine mais jamais engagés dans ce chantier).
