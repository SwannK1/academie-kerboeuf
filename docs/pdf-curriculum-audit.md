...
21. ~~Recherche transversale QLM/sciences/EMC pour CE1/CE2 (même modèle que pour CP).~~ Fait le 9 septembre 2026 : **aucun PDF QLM/sciences/EMC à CE1 ni CE2** (`public/fiches/ce1` ne contient que `francais/` ; `public/fiches/ce2` ne contient que `francais/` et `mathematiques/`). Cartographie programme vérifiée : `content/levels/ce1-competencies.ts` et `ce2-competencies.ts` incluent chacun exactement 2 compétences « Questionner le monde » (CE1 : états de la matière, cycle de l'eau ; CE2 : lire un plan, frise chronologique) — **toutes les 4 déjà marquées `status: "upcoming"` dans le code**, cohérent avec l'absence de PDF, pas une lacune cachée. Aucune compétence EMC mappée à CE1 ni CE2 (0 occurrence), même constat systémique que CP et CM1.
...
### Mathématiques 3e clos (6/6 compétences, 18/18 PDF) — 1 correction appliquée
...
### Arts plastiques, éducation musicale, EPS 4e clos (3/3 compétences, 9/9 PDF) — **DÉFAUT MAJEUR CONFIRMÉ : contenu en anglais dans trois matières françaises**

`arts-cadrage` (cadrage et point de vue photographique), `mus-comparer` (comparer deux œuvres musicales), `eps-strategie` (adapter sa stratégie à l'adversaire, exemple badminton). **Sur les 3 compétences et les 9 fichiers (leçon + exercices + évaluation de chacune), tout le contenu spécifique à la tâche (les encadrés « Savoir »/« JE RETIENS », « Exemple », et surtout la totalité des items de chaque Palier 1-4 en exercices ainsi que les 4 items de chaque évaluation) est rédigé en anglais**, alors qu'il s'agit de trois matières enseignées en français (arts plastiques, éducation musicale, EPS — pas des cours de langue). Seuls les éléments de gabarit partagés (« Note précisément ta réponse », « Justifie avec une trace vérifiable », les intitulés de rubriques et de barème) restent en français. Exemple concret : l'évaluation `arts-cadrage` demande en anglais « Produce an image titled « Fragile / puissant » » ; l'évaluation `eps-strategie` demande « Prepare and play a constrained opposition » ; l'évaluation `mus-comparer` demande « Compare two unfamiliar one-minute excerpts ».

**Confirmé systématique et non isolé** : identique sur les 3 compétences uniques de ces 3 matières (100 % du contenu propre à ces matières à ce niveau), suggérant une erreur de génération de contenu (paramètre de langue resté sur anglais pour ce lot), à ne pas confondre avec les fiches `anglais` elles-mêmes (correctement en anglais). Le contenu pédagogique sous-jacent (cadrage photographique, critères d'écoute musicale, stratégie sportive) est par ailleurs pertinent et bien conçu — **seul le choix de la langue est fautif**, pas le fond.

**Non corrigé dans ce lot** (correction sûre et bien définie — traduction directe des mêmes consignes — mais volumineuse : 9 fichiers × plusieurs encadrés chacun ; réservée à un lot de correction dédié plutôt que traitée au fil de l'audit, par cohérence avec la méthode suivie depuis 6e pour les autres défauts confirmés).
...
### Anglais 5e clos (5/5 compétences, 15/15 PDF) — **5E INTÉGRALEMENT CLOS**

`comprehension-ecrite` (texte sur un séjour d'échange, anglais britannique cohérent — « neighbours »), `comprehension-orale` (script sur un nettoyage de rive retardé par la pluie), `culture-comparer` (Glasgow/Toronto, méthodologie anti-stéréotype bien conçue : « sans stéréotype », « distingue observation et généralisation »), `expression-ecrite`, `expression-orale` (connecteurs temporels anglais tous corrects : first/while/suddenly/because/finally). Anglais grammaticalement correct sur l'ensemble des 5 fiches. **Défaut confirmé (récurrent, même nature que français 5e)** : `expression-ecrite`, évaluation item 3 (3 pts) demande de réviser un brouillon qui « repeats nice five times and changes from past to present without reason » — ce brouillon n'est jamais fourni dans le fichier. Non corrigé. Barèmes `/10` tous exacts (2+4+3+1).

**5E EST MAINTENANT INTÉGRALEMENT AUDITÉ : 5 matières, 66/66 PDF** (français 4/4, mathématiques 5/5, histoire-géographie-EMC 4/4, sciences-technologie 4/4, anglais 5/5 — 22 compétences au total). Bilan cumulé 5e : 0 PDF modifié, défauts confirmés uniquement sur le motif récurrent « brouillon/support à réviser non fourni » (français : 2 items, anglais : 1 item) — **aucune erreur factuelle, scientifique, historique, mathématique ou linguistique trouvée sur l'ensemble du niveau**, contrairement à 6e qui avait des défauts de fond (pronoms/adverbe, passé simple/composé, données génériques). 5e est le niveau le plus propre audité à ce stade du chantier.
...
### 6e Histoire-Géographie-EMC — domaine histoire clos (6/6 compétences, 18/18 PDF) — **6E INTÉGRALEMENT CLOS**

`source` (photo de tablette d'argile = source primaire vs récit rédigé aujourd'hui = travail d'historien, distinction correcte), `frise-simple` (frise 500 av.J.-C. à 500 apr.J.-C., graduation 250 ans, placements 250 av.J.-C./an 1/300 apr.J.-C. tous cohérents), `temps-long` (3300 av.J.-C. bien antérieur à l'an 1), `prelever-informations` (vérifié distinct de `source` : extraire une réponse vs identifier la nature du document), `expliquer-fait`, `raconter-evenement` (vérifié distinct de `expliquer-fait` : narration qui/quand/où vs explication cause/conséquence). Le fait historique central (invention de l'écriture vers 3300 av. J.-C.) est exact et cohérent sur les 6 compétences qui le réutilisent. Aucune erreur trouvée. Barèmes `/10` tous exacts.

**6E EST MAINTENANT INTÉGRALEMENT AUDITÉ : 62 compétences, 186/186 PDF** (français 22/22 = 66 PDF, mathématiques 28/28 = 84 PDF, histoire-géographie-EMC 12/12 = 36 PDF). Bilan cumulé 6e : 0 PDF modifié (limite : aucune n'a nécessité de correction jugée sûre et non ambiguë sans risque de dénaturer un choix pédagogique), défauts confirmés uniquement en français (écriture 1, étude de la langue 3) et en mathématiques (données de travail génériques transversales + supports de lecture manquants sur 4 compétences OGD/résolution-problèmes) ; **aucun défaut de contenu trouvé en histoire-géographie-EMC** ; confirmation systémique de l'absence totale d'EMC (0/12 compétences) cohérente avec CP→CM2.
